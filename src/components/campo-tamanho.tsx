...
export type TamanhoOpcao = {
  id: string;
  codigo: string;
  descricao: string;
};

export function CampoTamanho({
  opcoes,
  valor,
  onChange,
  disabled = false,
  ...
  const filtradas = useMemo(() => {
    ...
    return opcoes.filter((opcao) =>
      [opcao.codigo, opcao.descricao].some((texto) =>
      ...
      {aberto && !disabled && (
        <div className="absolute z-40 mt-1 max-h-56 w-full overflow-y-auto rounded-md border border-slate-200 bg-white shadow-lg">
          {filtradas.map((opcao) => (
            ...
            >
              <span className="block text-sm font-medium text-slate-900">{opcao.descricao}</span>
            </button>
          ))}
          {filtradas.length === 0 && (
