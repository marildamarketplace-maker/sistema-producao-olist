...
function CriarPedidoOlistPage() {
  ...
  return <div className="space-y-6">
    ...
    <ol className="grid grid-cols-3 gap-2">{["Vendedor", "Cliente", "Produtos"].map((nome, indice) => <li key={nome} className={`rounded-lg border px-2 py-3 text-center text-xs font-medium sm:px-4 sm:text-left sm:text-sm ${etapa === indice + 1 ? "border-slate-900 bg-slate-900 text-white" : etapa > indice + 1 ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-white text-slate-500"}`}>{indice + 1}. {nome}</li>)}</ol>
    ...
    {!sucesso && etapa === 3 && <form onSubmit={criarPedido} className="space-y-4 pb-24 sm:space-y-5 sm:pb-0"><section className="rounded-lg border border-slate-200 bg-white p-4 sm:p-6"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold text-slate-900">Adicionar produto</h3><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600">{itens.length} no pedido</span></div><div className="mt-4 grid grid-cols-[minmax(0,1fr)_96px] gap-3 md:grid-cols-[1fr_180px_auto] md:items-end"><label className="relative col-span-2 text-sm text-slate-700 md:col-span-1">Produto<input placeholder="Busque por nome ou SKU" value={buscas.produto} onChange={(e) => { setBuscas({ ...buscas, produto: e.target.value }); setProdutoId(""); setProdutoBuscaAberta(true); }} onFocus={() => setProdutoBuscaAberta(true)} onBlur={() => window.setTimeout(() => setProdutoBuscaAberta(false), 150)} className="mt-1 min-h-11 w-full rounded-md border border-slate-300 px-3 py-2" />{produtoBuscaAberta && <div className="absolute z-30 mt-1 max-h-[50vh] w-full overflow-y-auto overscroll-contain rounded-md border border-slate-200 bg-white shadow-xl">{buscando ? <div className="px-3 py-4 text-sm text-slate-500">Carregando produtos...</div> : produtosSugeridos.length > 0 ? produtosSugeridos.map((produto) => <button key={produto.id} type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => { setProdutoId(String(produto.id ?? "")); setBuscas({ ...buscas, produto: `${produto.sku ?? ""} — ${produto.descricao ?? ""}` }); setProdutoBuscaAberta(false); }} className="block min-h-14 w-full border-b border-slate-100 px-3 py-3 text-left last:border-0 hover:bg-slate-50"><span className="block text-sm font-semibold text-slate-900">{produto.sku ?? produto.id}</span><span className="mt-0.5 block line-clamp-2 text-xs text-slate-500">{produto.descricao ?? "Sem descrição"} · {produto.unidade ?? "Sem unidade"}</span></button>) : <div className="px-3 py-4 text-sm text-slate-500">Nenhum produto encontrado.</div>}</div>}</label><label className="text-sm text-slate-700">Quantidade<input aria-label="Quantidade do novo produto" type="number" min="1" step="0.01" inputMode="decimal" value={quantidadeNova} onChange={(e) => setQuantidadeNova(e.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-slate-300 px-3 py-2" /></label><button type="button" disabled={!produtoId} onClick={adicionarProduto} className="min-h-11 w-full rounded-md bg-slate-900 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50 md:w-auto">Adicionar</button></div><p className="mt-2 text-xs text-slate-500">{produtosFiltrados.length} de {dados.produtos.length} produtos disponíveis{produtosFiltrados.length > 100 ? " · refine a busca para ver resultados mais específicos" : ""}</p></section>
      {itens.map((item) => <section key={item.id} className="rounded-lg border border-slate-200 bg-white p-4 sm:p-6"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h4 className="truncate font-semibold text-slate-900">{item.produto.sku}</h4><p className="mt-1 line-clamp-2 text-sm text-slate-500">{item.produto.descricao}</p></div><button type="button" onClick={() => setItens((atuais) => atuais.filter((atual) => atual.id !== item.id))} className="shrink-0 rounded-md border border-red-200 px-3 py-2 text-sm font-medium text-red-600">Remover</button></div><div className="mt-4 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-5"><label className="text-sm text-slate-700">Quantidade<input type="number" min="1" step="0.01" inputMode="decimal" value={item.quantidade} onChange={(e) => atualizarItem(item.id, { quantidade: e.target.value })} className="mt-1 min-h-11 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700">Valor unitário<input type="text" inputMode="numeric" value={item.valorUnitario} onChange={(e) => atualizarItem(item.id, { valorUnitario: formatarValorCentavos(e.target.value) })} onFocus={(e) => e.currentTarget.select()} aria-label="Valor unitário em reais" className="mt-1 min-h-11 w-full rounded-md border border-slate-300 px-3 py-2 text-right tabular-nums" /></label><label className="col-span-2 text-sm text-slate-700 sm:col-span-1">Tipo do produto (TIPO)<input value={item.tipo} onChange={(e) => atualizarItem(item.id, { tipo: e.target.value })} placeholder="Ex.: LENCO-RELIG-FOURWAY" className="mt-1 min-h-11 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700">Tamanho (TAM)<input value={item.tamanho} onChange={(e) => atualizarItem(item.id, { tamanho: e.target.value })} placeholder="Ex.: 70x70" className="mt-1 min-h-11 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm text-slate-700">Corte a laser<select value={item.laser ? "true" : "false"} onChange={(e) => atualizarItem(item.id, { laser: e.target.value === "true" })} className="mt-1 min-h-11 w-full rounded-md border border-slate-300 bg-white px-3 py-2"><option value="false">Não</option><option value="true">Sim</option></select></label></div>{produtoSublime(item.produto) && <div className="mt-5 border-t border-slate-100 pt-4"><div><h5 className="text-sm font-semibold text-slate-800">Divisão por estampa</h5></div><div className="mt-3 space-y-3">{item.divisoes.map((divisao) => { const variantes = dados.variantes.filter((variante) => variante.estampaId === divisao.estampaId); return <div key={divisao.id} className="grid grid-cols-[96px_1fr] gap-3 rounded-md bg-slate-50 p-3 md:grid-cols-[120px_1fr_1fr_auto]"><input aria-label="Quantidade da divisão" type="number" min="0.01" step="0.01" inputMode="decimal" value={divisao.quantidade} onChange={(e) => atualizarDivisao(item.id, divisao.id, { quantidade: e.target.value })} className="min-h-11 rounded-md border border-slate-300 px-3 py-2" /><button type="button" disabled={item.divisoes.length === 1} onClick={() => atualizarItem(item.id, { divisoes: item.divisoes.filter((atual) => atual.id !== divisao.id) })} className="rounded-md border border-red-200 px-3 py-2 text-sm text-red-600 disabled:opacity-30 md:order-4">Excluir</button><div className="col-span-2 md:col-span-1"><CampoCodigo rotulo="Estampa" placeholder="Código da estampa" opcoes={dados.estampas} codigo={divisao.estampaCodigo} onChange={(estampaId, estampaCodigo) => atualizarDivisao(item.id, divisao.id, { estampaId, estampaCodigo, varianteId: "", varianteCodigo: "" })} /></div><div className="col-span-2 md:col-span-1"><CampoCodigo rotulo="Variante" placeholder="Código da variante" opcoes={variantes} codigo={divisao.varianteCodigo} onChange={(varianteId, varianteCodigo) => atualizarDivisao(item.id, divisao.id, { varianteId, varianteCodigo })} /></div></div>; })}<div className="flex justify-end"><button type="button" onClick={() => atualizarItem(item.id, { divisoes: [...item.divisoes, { id: novoId(), quantidade: "1", estampaId: "", estampaCodigo: "", varianteId: "", varianteCodigo: "" }] })} className="min-h-11 rounded-md border border-dashed border-blue-300 px-4 py-2 text-sm font-medium text-blue-700">+ Adicionar divisão</button></div></div><p className={`mt-2 rounded-md px-3 py-2 text-xs font-medium ${divisaoValida(item) ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}>Total dividido: {item.divisoes.reduce((total, divisao) => total + (numero(divisao.quantidade) || 0), 0)} de {item.quantidade}</p></div>}</section>)}
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><button type="button" onClick={() => setEtapa(2)} className="w-full rounded-md border border-slate-300 px-5 py-2.5 text-sm sm:w-auto">Voltar</button><button disabled={enviando || !itensValidos} className="hidden rounded-md bg-emerald-700 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-50 sm:block">{enviando ? "Criando pedido..." : "Criar pedido na Olist"}</button></div>
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-[0_-8px_24px_rgba(15,23,42,0.12)] backdrop-blur sm:hidden"><div className="mx-auto flex max-w-lg items-center gap-3"><div className="min-w-0 flex-1"><p className="text-xs text-slate-500">{itens.length} {itens.length === 1 ? "produto" : "produtos"}</p><p className="truncate text-sm font-semibold text-slate-900">{totalPedidoFormatado}</p></div><button disabled={enviando || !itensValidos} className="min-h-11 rounded-md bg-emerald-700 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-50">{enviando ? "Criando..." : "Criar pedido"}</button></div></div>
    </form>}
  </div>;
}
