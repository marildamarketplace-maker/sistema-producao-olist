...
function CampoEstampa({ estampas, estampaId, onChange }: { estampas: Estampa[]; estampaId: string; onChange: (id: string) => void }) {
  const selecionada = estampas.find((estampa) => estampa.id === estampaId);
  const [busca, setBusca] = useState(selecionada?.codigo ?? "");
  const [aberto, setAberto] = useState(false);
  const termo = busca.trim().toLocaleLowerCase("pt-BR");
  const filtradas = estampas.filter((estampa) =>
    !termo || [estampa.codigo, estampa.descricao].some((valor) => String(valor ?? "").toLocaleLowerCase("pt-BR").includes(termo)),
  );

  return <div className="relative"><input aria-label="Estampa" placeholder="Digite o código da estampa" value={busca} onFocus={() => setAberto(true)} onBlur={() => window.setTimeout(() => setAberto(false), 150)} onChange={(event) => { setBusca(event.target.value); onChange(""); setAberto(true); }} className="w-full rounded-md border border-slate-300 px-3 py-2" />{aberto && <div className="absolute z-30 mt-1 max-h-56 w-full overflow-y-auto rounded-md border border-slate-200 bg-white shadow-lg"><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { setBusca(""); onChange(""); setAberto(false); }} className="block w-full px-3 py-2 text-left text-sm text-slate-500 hover:bg-slate-50">Sem estampa</button>{filtradas.map((estampa) => <button key={estampa.id} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { setBusca(estampa.codigo); onChange(estampa.id); setAberto(false); }} className="block w-full border-t border-slate-100 px-3 py-2 text-left hover:bg-slate-50"><span className="block text-sm font-medium text-slate-900">{estampa.codigo}</span>{estampa.descricao && <span className="block text-xs text-slate-500">{estampa.descricao}</span>}</button>)}</div>}</div>;
}
...
function CriarPedidoOlistPage() {
  ...
  useEffect(() => {
    if (etapa !== 1) return;
    const atual = { recurso: "vendedores" as const, termo: buscas.vendedor };
  ...
  return <div className="space-y-6">
    ...
    {!sucesso && etapa === 2 && <section className="rounded-lg border border-slate-200 bg-white p-4 sm:p-6"><h3 className="font-semibold text-slate-900">Selecione o cliente</h3><input placeholder="Filtrar por nome, código ou CPF/CNPJ" value={buscas.cliente} onChange={(e) => setBuscas({ ...buscas, cliente: e.target.value })} className="mt-4 w-full rounded-md border border-slate-300 px-3 py-2" /><p className="mt-2 text-xs text-slate-500">{buscando ? "Carregando todos os clientes do vendedor..." : `${clientesFiltrados.length} de ${dados.clientes.length} clientes`}</p><div className="mt-4 max-h-[28rem] overflow-auto rounded-md border border-slate-200"><table className="min-w-full text-sm"><thead className="sticky top-0 bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="w-12 px-4 py-3">Sel.</th><th className="px-4 py-3">Cliente</th><th className="px-4 py-3">CPF/CNPJ</th><th className="px-4 py-3">Código</th><th className="px-4 py-3">ID</th></tr></thead><tbody className="divide-y divide-slate-100">{clientesFiltrados.map((cliente) => { const id = String(cliente.id ?? ""); const selecionado = clienteId === id; return <tr key={id} onClick={() => setClienteId(id)} className={`cursor-pointer ${selecionado ? "bg-emerald-50" : "hover:bg-slate-50"}`}><td className="px-4 py-3"><input type="radio" name="cliente" checked={selecionado} onChange={() => setClienteId(id)} /></td><td className="min-w-52 px-4 py-3 font-medium text-slate-900">{cliente.nome ?? cliente.fantasia ?? `Cliente ${cliente.id}`}</td><td className="whitespace-nowrap px-4 py-3 text-slate-600">{cliente.cpfCnpj ?? "—"}</td><td className="px-4 py-3 text-slate-600">{cliente.codigo ?? "—"}</td><td className="px-4 py-3 text-slate-600">{cliente.id ?? "—"}</td></tr>; })}{!buscando && clientesFiltrados.length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-slate-500">Nenhum cliente encontrado.</td></tr>}</tbody></table></div><div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><button onClick={() => setEtapa(1)} className="w-full rounded-md border border-slate-300 px-5 py-2 text-sm sm:w-auto">Voltar</button><button disabled={!clienteId} onClick={() => setEtapa(3)} className="w-full rounded-md bg-slate-900 px-5 py-2 text-sm font-medium text-white disabled:opacity-50 sm:w-auto">Continuar</button></div></section>}
