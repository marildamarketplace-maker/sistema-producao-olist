...
export default function PedidosOlistPage() {
  const { session } = useAuth();
  const iniciais = { numero: "", nomeCliente: "", cpfCnpj: "", numeroPedidoEcommerce: "", dataInicial: "", dataFinal: "", situacao: "", origemPedido: "", orderBy: "desc" };
  ...
  return <div className="space-y-6">
    ...
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><h3 className="font-semibold text-slate-900">Pedidos</h3><span className="text-sm text-slate-500">{total.toLocaleString("pt-BR")} encontrados</span></div>
      {carregando ? <p className="p-6 text-sm text-slate-600">Consultando a Olist...</p> : itens.length === 0 ? <p className="p-6 text-sm text-slate-600">Nenhum pedido encontrado.</p> : <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Pedido</th><th className="px-4 py-3">E-commerce</th><th className="px-4 py-3">Cliente</th><th className="px-4 py-3">Criação</th><th className="px-4 py-3">Situação</th><th className="px-4 py-3">Valor</th></tr></thead><tbody className="divide-y divide-slate-100">{itens.map((pedido, indice) => { const situacao = texto(pedido, "situacao", "idSituacao"); return <tr key={`${texto(pedido, "id", "numero", "numeroPedido")}-${indice}`}><td className="whitespace-nowrap px-4 py-3"><div className="font-medium text-slate-900">#{texto(pedido, "numero", "numeroPedido", "id")}</div><div className="text-xs text-slate-500">ID {texto(pedido, "id")}</div></td><td className="whitespace-nowrap px-4 py-3 text-slate-600">{texto(pedido, "numeroPedidoEcommerce", "numeroEcommerce")}</td><td className="min-w-56 px-4 py-3"><div>{texto(pedido, "nomeCliente", "cliente")}</div><div className="text-xs text-slate-500">{texto(pedido, "cpfCnpj", "cpfCnpjCliente")}</div></td><td className="whitespace-nowrap px-4 py-3 text-slate-600">{dataFormatada(texto(pedido, "dataCriacao", "dataPedido", "data"))}</td><td className="whitespace-nowrap px-4 py-3"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs">{SITUACOES[situacao] ?? situacao}</span></td><td className="whitespace-nowrap px-4 py-3 font-medium">{valorFormatado(texto(pedido, "valor", "valorTotal", "total"))}</td></tr>; })}</tbody></table></div>}
